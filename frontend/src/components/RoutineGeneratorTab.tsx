...
export default function RoutineGeneratorTab({
  onRoutineGenerated,
  hasExistingRoutines,
    ...
    try {
      ...
      setErrorHeader(
        err.response?.data?.error ||
          "A critical exception was thrown during backtracking execution.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleClear = async () => {
    if (
      !window.confirm(
        "Are you strictly sure you want to clear the entire generated university class routine? This is irreversible.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setErrorHeader(
        err.response?.data?.error ||
          "Failed to clear current routine database.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6" id="generator-section">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">
          Compute and draft conflict-free university class schedules matching
          standard educational constraints.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white p-5 rounded-lg border border-gray-200 shadow-xs flex flex-col justify-between h-fit gap-4">
          <div>
            <h3 className="text-sm font-semibold text-gray-700 pb-2 border-b border-gray-100 flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Sliders className="w-4 h-4 text-sky-600" />
                Engine Operations
              </div>
              <span title="Backtracking engine scans overall courses and builds timetable slots with zero conflicts">
                <HelpCircle className="w-4 h-4 text-gray-400 shrink-0 cursor-help" />
              </span>
            </h3>

            <p className="text-xs text-gray-600 leading-relaxed mb-4">
              Our automated system uses a backtracking search algorithm with
              constraint propagation. The system scans faculties, student
              capacities, and facility allocations instantly to compute safe
              calendars.
            </p>
          ...
          <div className="space-y-2 pt-2 border-t border-gray-100">
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 bg-[#2C4A6F] text-white text-xs font-semibold py-3 px-4 rounded hover:bg-[#1B324F] transition disabled:opacity-50 cursor-pointer"
            >
              {loading ? (
            ...
            {hasExistingRoutines && (
              <button
                onClick={handleClear}
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 bg-white text-red-600 border border-red-200 text-xs font-semibold py-2.5 px-4 rounded hover:bg-rose-50 transition cursor-pointer"
              >
            ...
            <div className="p-4 bg-emerald-50 border-l-4 border-emerald-500 rounded text-emerald-800 text-xs flex items-start gap-3 shadow-xs">
              ...
              <div>
                <span className="font-semibold block">
                  Academic Generation Successful!
                </span>
                <span className="mt-1 flex items-center gap-1.5 flex-wrap">
                  {successMsg} Go to the Routine Viewer to check results.
                  <ArrowRight className="w-3.5 h-3.5 text-emerald-600 animate-pulse shrink-0" />
                </span>
              </div>
            </div>
          )}
            ...
            <div className="p-4 bg-red-50 border-l-4 border-red-500 rounded text-red-800 text-xs flex flex-col gap-2 shadow-xs">
              ...
              {conflicts && conflicts.length > 0 && (
                <div className="mt-3 bg-white p-3 rounded border border-red-100">
                  <span className="text-[10px] uppercase font-bold tracking-wider text-rose-800 block mb-2">
                    Backtracking Failure Analysis:
                  </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1 mb-2">
                  ...
                  Working Academic Days:
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {formatDays.map((d, i) => (
                    <span
                      key={i}
                      className="px-3 py-1.5 bg-slate-50 border border-gray-200 rounded text-xs font-medium text-gray-700"
                    >
                      {d}
                    </span>
                  ))}
                ...
                <span className="text-[10px] text-gray-400 block mt-2">
                  Routine spans Friday & Saturday as official weekend holidays.
                </span>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1.5 max-h-48 overflow-y-auto pr-2">
                  {formatSlots.map((s, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center text-xs p-2 bg-gray-50 border border-gray-100 rounded"
                    >
